...
import Ventanas.Main; // Importa la clase Main
...
/**
 *
 * @author Jheyson Jhostin Manuel
 */


public class VentanaPrincipal extends javax.swing.JFrame{
       
    private JPanel mainPanel; // JPanel para mostrar el contenido del Main
    private Main mainFrame;   // Instancia de la clase Main
   ...
   public static void R_repaint(Graphics g, int tope, Arboles arboles) {
     // Pinta las líneas y los círculos del grafo
    for (int j = 0; j < tope; j++) {
        for (int k = 0; k < tope; k++) {
            if (arboles.getmAdyacencia(j, k) == 1) {
                Pintar.pintarLinea(g, arboles.getCordeX(j), arboles.getCordeY(j), arboles.getCordeX(k), arboles.getCordeY(k), arboles.getmCoeficiente(j, k));
            }
        }
    }
    for (int j = 0; j < tope; j++) {
        Pintar.pintarCirculo(g, arboles.getCordeX(j), arboles.getCordeY(j), String.valueOf(arboles.getNombre(j)));
    }
}
 
public static int ingresarNodoOrigen(String nodoOrigenPrompt, String noExistePrompt, int tope) {
    int nodoOrigen = -1; // Valor por defecto inválido
    boolean inputValido = false;

    while (!inputValido) {
        String input = JOptionPane.showInputDialog(nodoOrigenPrompt);
        
        if (input == null) {
            return -1; // Usuario cerró la ventana, finaliza el evento
        }
        
        try {
            nodoOrigen = Integer.parseInt(input);
            if (nodoOrigen >= tope) {
                JOptionPane.showMessageDialog(null, noExistePrompt + "\nDebe ingresar un Nodo existente");
            } else {
                inputValido = true; // La entrada es válida, sal del bucle
            }
        } catch (NumberFormatException ex) {
            // Manejar la excepción de entrada no numérica (continuar en el bucle)
        }
    }
    ...
    for (int j = 0; j < tope; j++) {
        if ((xxx + 2) > arboles.getCordeX(j) && xxx < (arboles.getCordeX(j) + 13) && (yyy + 2) > arboles.getCordeY(j) && yyy < (arboles.getCordeY(j) + 13)) {
            if (n == 0) {
                ...
                if (id == id2) {
                    n = 0;
                    R_repaint(jPanel1.getGraphics(), tope, arboles); // Aquí utilizamos R_repaint
                    id = -1;
                    id2 = -1;
                }
            }
...
public void manejarClicSobreNodo(int nodoIndex) {
    Graphics g = jPanel1.getGraphics();
    if (n == 0) {
        id = nodoIndex;
        R_repaint(g, tope, arboles); // Aquí utilizamos R_repaint
        Pintar.clickSobreNodo(g, arboles.getCordeX(nodoIndex), arboles.getCordeY(nodoIndex), String.valueOf(arboles.getNombre(nodoIndex)), Color.orange);
    } else {
        id2 = nodoIndex;
        n++;
        Pintar.clickSobreNodo(g, arboles.getCordeX(nodoIndex), arboles.getCordeY(nodoIndex), String.valueOf(arboles.getNombre(nodoIndex)), Color.orange);
 ...
 public void clicIzqSobreNodo(int xxx, int yyy) {
    for (int j = 0; j < tope; j++) {
        if ((xxx + 2) > arboles.getCordeX(j) && xxx < (arboles.getCordeX(j) + 13) && (yyy + 2) > arboles.getCordeY(j) && yyy < (arboles.getCordeY(j) + 13)) {
            if (nn == 0) {
                permanente = j;
                R_repaint(jPanel1.getGraphics(), tope, arboles); // Utilizamos R_repaint
            } else {
                nodoFin = j;
            }
            nn++;
            n = 0;
            id = -1;
            Pintar.clickSobreNodo(jPanel1.getGraphics(), arboles.getCordeX(j), arboles.getCordeY(j), String.valueOf(arboles.getNombre(j)), Color.GREEN);  
            break;
        }
    }
 }
 ...
 jmapa.setIcon(new ImageIcon(scaledInstance));  
    ...
    public VentanaPrincipal() { 
        ...
        //Crea el JPanel para mostrar el contenido del Main
        mainPanel = new JPanel();
        mainFrame = new Main(); // Crea una instancia del Main
       // mapPanel=new MapPanel();
        // Agrega el contenido del Main al JPanel
        mainPanel.add(mainFrame.getContentPane());
        
        // Agrega el JPanel a la VentanaPrincipal
    ...
    private void initComponents() {
        ...
        JMenuMaps = new javax.swing.JMenu();
        ItemMaps = new javax.swing.JMenuItem();
        ...
        jPanel1.add(jmapa);
        jmapa.setBounds(0, 0, 820, 480);

        getContentPane().add(jPanel1);
        jPanel1.setBounds(200, 20, 820, 480);

        jButton1.setText("Limpiar Camino");
        ...
        jButton1.setBounds(30, 360, 110, 40);
        ...
        jButton16.setBounds(30, 100, 60, 23);
        ...
        jPanel3.setBounds(20, 210, 130, 60);

        jButton15.setText("muestra 1");
        ...
        jPanel4.setBounds(20, 280, 140, 40);
        getContentPane().add(jLabelTime);
        jLabelTime.setBounds(10, 420, 190, 40);
        getContentPane().add(jLMemoria);
        jLMemoria.setBounds(30, 490, 170, 50);
        ...
        jButton14.setBounds(270, 40, 60, 23);

        jLabelAcerca.setBorder(new javax.swing.border.MatteBorder(null));
        jPanel5.add(jLabelAcerca);
        jLabelAcerca.setBounds(0, 0, 360, 90);

        getContentPane().add(jPanel5);
        jPanel5.setBounds(460, 500, 360, 90);
        ...
        jMenu4.setText("Muestras");

        jMenuItem14.setText("Muestra1");
        ...
        JMenuMaps.setText("Maps");

        ItemMaps.setText("Cargar Maps");
        ItemMaps.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                ItemMapsActionPerformed(evt);
            }
        });
        JMenuMaps.add(ItemMaps);

        jMenuBar1.add(JMenuMaps);

        setJMenuBar(jMenuBar1);

        setSize(new java.awt.Dimension(1050, 648));
...
if(tope < 1) {
    ...
    // Llamar al garbage collector para minimizar el ruido en las mediciones
    runtime.gc();

    // Medir la memoria total y la memoria libre antes de ejecutar el algoritmo
    ...
    long tiempoTotal = (fin - inicio) / 1_000_000; // para obtener milisegundos

    jtacumulado.setText("" + Prim.getCumulado());

    // Muestra el tiempo que tardó el algoritmo en ejecutarse
    jLabelTime.setText("Tiempo de Prim: " + tiempoTotal + "ms");

    // Llamar al garbage collector nuevamente
    runtime.gc();

    // Medir la memoria total y la memoria libre después de ejecutar el algoritmo
    long memoryUsedAfter = runtime.totalMemory() - runtime.freeMemory();

    // Calcular la diferencia de memoria utilizada
    long memoryDifference = memoryUsedAfter - memoryUsedBefore;
    jLMemoria.setText(String.valueOf(memoryDifference) + " Bytes");
}
    ...
    private void jMenuItem2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jMenuItem2ActionPerformed
      if (tope <= 1) {
        ...
        this.setEnabled(false);
        VentanaArista ventanaArista = new VentanaArista(arboles, pintar, tope, this);
        ventanaArista.setVisible(true);
        ventanaArista.addWindowListener(new WindowAdapter() {
            @Override
            public void windowClosed(WindowEvent e) {
                jPanel1.paint(jPanel1.getGraphics());
                R_repaint(jPanel1.getGraphics(), tope, arboles); // Redibujar el grafo
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
       Graphics g = jPanel1.getGraphics(); // Obtener el contexto gráfico
    R_repaint(g, tope, arboles); // Redibujar el grafo
    jtacumulado.setText(""); // Restablecer el campo de texto
    ...
    if (Eliminar <= tope && Eliminar >= 0 && tope > 0) {
        ...
        tope--;
        jPanel1.paint(jPanel1.getGraphics());
        R_repaint(jPanel1.getGraphics(), tope, arboles);
    }
...
jPanel1.setBackground(jButton8.getBackground());        // TODO add your handling code here:
    ...
    private void jMenuItem13ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jMenuItem13ActionPerformed
        ...
        // Crear una instancia de la clase Algoritmo_Dijkstra y ejecutar el algoritmo
        Algoritmo_Dijkstra Dijkstra = new Algoritmo_Dijkstra(arboles, tope, permanente, nodoFin);
        Dijkstra.dijkstra();

        // Calcular el tiempo total y mostrarlo en un JLabel
        long fin = System.nanoTime();
        long tiempoTotal = (fin - inicio) / 1_000_000; // Convertir nanosegundos a milisegundos
        jLabelTime.setText("Tiempo de Dijkstra: " + tiempoTotal + "ms");
        ...
        long memoryUsed = (totalMemoryBefore - freeMemoryBefore) - (totalMemoryAfter - freeMemoryAfter);
        jLMemoria.setText(String.valueOf(memoryUsed) + " Bytes");

        // Mostrar el valor del acumulado en un JTextField
        jtacumulado.setText("" + Dijkstra.getAcumulado());
    } else {
        JOptionPane.showMessageDialog(null, "Se deben crear más nodos...");
    }
    ...
    private void jButton15ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton15ActionPerformed
     ...
     jPanel1.paint(jPanel1.getGraphics()); // Limpia el panel antes de redibujar

    Random rand = new Random();
    int numNodos = 6; // Ajusta la cantidad de nodos según tus necesidades
    int maxCoeficiente = 100; // Valor máximo que puede tener un coeficiente
    int espaciadoEntreNodos = 150; // Ajusta el espaciado entre nodosf

    int Matriz[][] = new int[numNodos][numNodos];
    int coe[][] = new int[numNodos][numNodos];
    int xx1[] = new int[numNodos];
    int yy1[] = new int[numNodos];
    int nom[] = new int[numNodos];

    int maxX = 950; // Ancho del panel
    int maxY = 630;// Alto del panel

    // Crear coordenadas aleatoriamente con espaciado entre nodos
    for (int i = 0; i < numNodos; i++) {
        xx1[i] = rand.nextInt(maxX - espaciadoEntreNodos);
        yy1[i] = rand.nextInt(maxY - espaciadoEntreNodos);
        nom[i] = i;
    }

    aristaMayor = 600; // Este valor puede ser ajustado si es necesario

    // Crear conexiones y coeficientes aleatoriamente
    for (int i = 0; i < numNodos - 1; i++) {
        Matriz[i][i + 1] = 1;
        Matriz[i + 1][i] = 1;
        coe[i][i + 1] = rand.nextInt(maxCoeficiente) + 1; // 1 a maxCoeficiente
        coe[i + 1][i] = coe[i][i + 1];
    }

    for (int i = 0; i < numNodos; i++) {
        for (int j = i + 2; j < numNodos; j++) {
            if (rand.nextInt(2) == 1) { // 50% de probabilidad de conexión
                Matriz[i][j] = 1;
                Matriz[j][i] = 1;
                coe[i][j] = rand.nextInt(maxCoeficiente) + 1; // 1 a maxCoeficiente
                coe[j][i] = coe[i][j];
            }
        }
    }
    ...
    String labelText = "<html><center>Elaborado por<br>" +
                      "Espinoza Jose Manuel<br>" +
                      "Jhostin Fabrizio Almerco Machado<br>" +
                      "Jheyson Jhordy Huarauya Riquelme<br>2023</center></html>";
    jLabelAcerca.setText(labelText);
    }//GEN-LAST:event_AcercaActionPerformed
    // Crea una instancia de GoogleMaps (esto podría estar en el constructor de tu clase o en algún otro método de inicialización)
    //Para Google maps
    
    private void ItemMapsActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ItemMapsActionPerformed
// Verificar si el frame Main ya está creado y visible
    if (mainFrame == null || !mainFrame.isVisible()) {
        // Crear una instancia del frame Main si aún no está creado
        if (mainFrame == null) {
            mainFrame = new Main();
        }
        
        // Mostrar el frame Main
        mainFrame.setVisible(true);
    }
    }//GEN-LAST:event_ItemMapsActionPerformed
    ...
    private javax.swing.JMenuItem ItemMaps;
    private javax.swing.JMenu JMenuMaps;
